...
void scatter(double THI, double *L_mu, double *L_p, double *L_v, long *idum){
	...
	// printf("count1 = %d, count2 = %d, count3 = %d x = %lf, a = %lf \n", count1, count2, count3, x, a);
...
/* generate several positions (z) and test the distribution */
void test_position(double *y1H, double *y1He, double *dy1H, double *Te, double y1H1, int n_photon, long *idum){
	double z;
...
/* generate several frequencies (dv) at selected temperature and test the distribution */
void test_frequency(int n_photon, long *idum){
	double dv;
	double THI = 5000; // can be modified, in [K]
...
/* generate several propagating distance (dz) at selected setting and test the distribution */
void test_propagate(int n_photon, double y1H1, int U, long *idum){
	double dz;
	double mu;
	...
	for(int i = 0; i < n_photon; i++){
		mu = 2 * ran2(idum) - 1;
		dz = propagate(L_v, mu, THI, y1H1, U, idum);
		printf("%lf \n", dz);
	}
}

/* generate scattering events at a few examples of settings and test the distribution */
void test_scatter(int n_photon, long *idum){
	/* can be modified */
	double THI = 5000;
	double L_mu = -0.5;
	double L_p = -0.5;
	...
	for(int i = 0; i < n_photon; i++){
		scatter(THI, &L_mu, &L_p, &L_v, idum);
		printf("%lf %lf %le \n", L_mu, L_p, L_v);
	}
}

/* test of each step, comment out if not needed */
void test_all(int n_photon, int z, double *L_position, double *L_mu, double *L_p, double *L_v,
	double *y1H, double *y1He, double *dy1H, double *Te, double *THI, double y1H1, int U, long *idum){
	float b; // random number
	double dz; // distance moved in number of grid
	double s; // distance moved in cm
	double p_i, p_f; // initial position and final position
	long count; // index for testing

	/* test each step of initializing, propagating and scattering once 
	printf("i, position, direction, polarization, frequency, propagate, new_direction, new_polarization, new_frequency\n");
	for(int i = 0; i < n_photon; i++){
		...
		printf("%d %d %lf %lf %le ", i, z, L_mu[i], L_p[i], L_v[i]);
	...
	/* test the whole processes until Lyman alpha photons escaped */
	...
	for(int i = 0; i < n_photon; i++){
		...
		// printf("%d %d %lf %lf %le \n", i, z, L_mu[i], L_p[i], L_v[i]);
		// printf("position, direction, polarization, frequency\n"); // record the status after every scattering process of one Lyman alpha photon
		...
		while((L_position[i] >= 0 && L_position[i] < NGRID) && fabs(L_v[i]) <= 1e13){
			...
			fflush(stdout);
			scatter(THI[z], &L_mu[i], &L_p[i], &L_v[i], idum);
			// printf("%lf %lf %lf %le \n", L_position[i], L_mu[i], L_p[i], L_v[i]); // record the status after every scattering process of one Lyman alpha photon
			fflush(stdout);
		}
		// printf("count = %d \n", count);
	}
...
int main(int argc, char **argv){
	...
	/* comment out any unnecessary steps */
	// test_position(y1H, y1He, dy1H, Te, y1H1, n_photon, idum);
	// test_frequency(n_photon, idum);
	// test_propagate(n_photon, y1H1, U, idum);
	// test_scatter(n_photon, idum);
	test_all(n_photon, z, L_position, L_mu, L_p, L_v, y1H, y1He, dy1H, Te, THI, y1H1, U, idum);
