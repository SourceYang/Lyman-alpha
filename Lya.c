...
#define C 29979245800.0 // speed of light, in [cm s^-1]
...
#define H0 2.18428524e-18 // current Hubble constant in the period of z = 8, in [s^-1]
#define OMEGA_M 0.315 // matter density
#define OMEGA_LAMBDA 0.685 // cosmological constant
#define EPS_Z 1e-9 // epsilon z

#define NGRID 200000 // number of grid
#define N_NU 128 // number of frequency bins
#define N_PHOTON_MAX 1048576 // occupy the max space
#define V_MAX 2e13 // max v away from v_center

/* get a random position of Lyman alpha photon, in grid */
int get_position(double *y1H, double *y1He, double *dy1H, double *Te, double y1H1, unsigned long long int idum){
...
double get_frequency(double THI, unsigned long long int idum){
...
double propagate(double dv, double mu, double THI, double y1H1, double y1H, int U, unsigned long long int idum){
	/* calculate the Voigt distribution */
	double delta_d; // doppler width
	double x;
	double a;
	double H_x;
	...
	/* this formula is derived from http://articles.adsabs.harvard.edu/pdf/1948ApJ...108..112H */
	delta_d = sqrt(2 * k * THI / M_HI) * v_center / C;
	x = dv / delta_d;
	a = 1 / (4 * M_PI * T_2p) / delta_d;
	H_x = H_xa(x, a);

	sigma_tot = C / (sqrt(2 * k * THI / M_p) * v_center) / sqrt(M_PI) * 3 * A * C * C * H_x / (8 * M_PI * (dv + v_center) * (dv + v_center));

	l_mfp = 1 / (y1H1 * y1H * sigma_tot);
	b = ran2(idum);
	s = l_mfp * log(1 / b);
	dz = s * mu - U * s / C;
	length = DNHI / y1H1;
...
void scatter(double THI, double *L_mu, double *L_p, double *L_v, unsigned long long int idum){
	double r, r1, r2; // random numbers
	...
	w1 = exp(-u_1b * u_1b) / ((u_1b - x) * (u_1b - x));
	w3 = exp(-u_1c * u_1c) / ((u_1c - x) * (u_1c - x));
	...
	if(fabs(x) > 2){
		...
		h_5 = exp(-u_1e * u_1e) / ((u_1e - x) * (u_1e - x));
...
int main(int argc, char **argv){
	...
	double y1H[NGRID], y1He[NGRID]; // neutral fractions of Hydrogen and Helium
	...
	double L_v_off[N_PHOTON_MAX], L_s_last[N_PHOTON_MAX], L_vi[N_PHOTON_MAX], L_vf[N_PHOTON_MAX]; // frequency offset, position where the photon last scattered, dv before and after last scattering
	...
	int z_center; // front location where y1H first reaches 0.5
	double b; //random number
	...
	unsigned long long int idum;  // random seed

	sscanf(argv[1], "%lf", &Z);
	sscanf(argv[2], "%d", &T);
	sscanf(argv[3], "%d", &U);
	sscanf(argv[4], "%lE", &y1H1);
	sscanf(argv[5], "%s", filename);
	sscanf(argv[6], "%d", &n_photon);
	sscanf(argv[7], "%ld", &idum);

	H = H0 * sqrt(OMEGA_M * (1+Z) * (1+Z) * (1+Z) + OMEGA_LAMBDA); // Hubble constant with redshift
	...
	for(int i = 0; i < n_photon; i++){
		...
		while((L_position[i] >= 0 && L_position[i] < NGRID) && fabs(L_v[i]) <= V_MAX){
			p_i = L_position[i]; // initial position before propagating
			z = (int)L_position[i];
			dz = propagate(L_v[i], L_mu[i], THI[z], y1H1, y1H[z], U, idum);
			L_position[i] += dz;

			if((L_position[i] > z && L_position[i] < z + 1) && (L_position[i] >= 0 && L_position[i] < NGRID)){
				z = (int)L_position[i];
				s = ((L_position[i] - p_i) * DNHI / y1H1) / (L_mu[i] - U / C);
				if(s > DNHI / y1H1){
					s = DNHI / y1H1;
					L_v[i] -= H * s * (v_center + L_v[i]) / C;
					L_position[i] = p_i + s * (L_mu[i] - U / C) * y1H1 / DNHI;
					continue;
				}
				L_v[i] -= H * s * (v_center + L_v[i]) / C;
				if(fabs(L_v[i]) <= V_MAX){
			...
			if(L_position[i] <= z || L_position[i] >= z + 1){
				if(L_position[i] <= z){
		...
		L_v_off[i] = L_v[i] - H * DNHI / y1H1 * (z_center - L_position[i]) / (y1H1 * (C * L_mu[i] - U)) * v_center;
		
		printf("%d %lf %lf %lf %le %le %lf %le %le \n", i, L_position[i], L_mu[i], L_p[i], L_v[i], L_v_off[i], L_s_last[i], L_vi[i], L_vf[i]);
	}
