...
int get_position(double *y1H, double *y1He, double *dy1H, double *Te, double y1H1, unsigned long long int idum){
	...
	for(i = 0; i < NGRID; i++){
		...
		qL[i] = get_Lya_rate(Te[i]);
		L_emissivity[i] = dy1H[i] * y1H1 * y1H1 * q[i] * qL[i];
...
double propagate(double dv, double mu, double THI, double y1H1, int U, unsigned long long int idum){
...
int main(int argc, char **argv){
	...
	double L_v_off[N_PHOTON_MAX], L_s_last[N_PHOTON_MAX]; // frequency offset, position where the photon last scattered
	...
	for(int i = 0; i < n_photon; i++){
		...
		while((L_position[i] >= 0 && L_position[i] < NGRID) && fabs(L_v[i]) <= 1e13){
			...
			if((L_position[i] >= 0 && L_position[i] < NGRID) && fabs(L_v[i]) <= 1e13){
				L_s_last[i] = L_position[i];
				scatter(THI[z], &L_mu[i], &L_p[i], &L_v[i], idum);
			}
		}

		L_v_off[i] = L_v[i] - L_v_off[i];
		
		printf("%d %lf %lf %lf %le %le %lf \n", i, L_position[i], L_mu[i], L_p[i], L_v[i], L_v_off[i], L_s_last[i]);
	}

	/* display 
	printf("i, position, direction, polarization, frequency, frequency offset, last position\n");
	for(int i = 0; i < n_photon; i++){
		printf("%d %lf %lf %lf %le %le %lf \n", i, L_position[i], L_mu[i], L_p[i], L_v[i], L_v_off[i], L_s_last[i]);
	} */
