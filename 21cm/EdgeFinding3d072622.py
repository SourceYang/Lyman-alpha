...
def TwoDContours(box1):
    ...
    for z in range(0,size,1):
       ...
       # uni_xy,uni_index = np.unique(xylists2,axis=0,return_index=True)
        ...
        #willapp =[]
        #print(uni_xy)
        #for unii in range(len(uni_xy)):
        #    willapp.append(uni_xy[unii])
        #pairslist.append(uni_xy.tolist())
...
def ThreeDlineslist(pairslist,size):
    ...
    for t in range(0,size,1): #z index
        ...
        for p in range(0,len(pairslist[t]),1):
            ...
            #y2=pairslist[t][p][4]
            check=False
            for trialp in range(0,p,1):
                xtrial=pairslist[t][trialp][0]
                ytrial=pairslist[t][trialp][1]
                ztrial=pairslist[t][trialp][2]
                
                if x1==xtrial and y1==ytrial and z1==ztrial:
                    check=True
            if check==False:
                xtemp=0
                ytemp=0
                ...
                #    linestemp.append((x1,y1,z1,x1,y1,z1))
                 #this happened 4 times for z=0
                 #what would this actually contribute to power spectrum
             #if temp<=1:
...
def flatTop2(zslice,pairslist, baseleft, baseright, internalTrianglesTop,w):
    ...
    for pa in range(len(pairslist[zslice])):
        if((pairslist[zslice][pa][3],pairslist[zslice][pa][4],pairslist[zslice][pa][2])==baseleft):
           ...
           if ((baseleft[0],baseleft[1],baseleft[2],baseright[0],baseright[1]) in pairslist[zslice]):
               pairslist[zslice].remove((baseleft[0],baseleft[1],baseleft[2],baseright[0],baseright[1]))
               pairslist[zslice].append((100000,100000,100000,100000,10000))   
...
def findFronts(pairslist,lines,size):
    ...
    #for i in range(0,3,1): #i indexes zslice
    for i in range(0,size,1):
        trianglecenters=[]
        for j in range(0,len(pairslist[i]),1):
                        ...
                        if up1 == (x1,y1,z1):
                            ...
                            for ll in range(0,len(lines[i]),1):
                                if (lines[i][ll][0],lines[i][ll][1],lines[i][ll][2])==bottomright:
                                    ...
                                    #if up2==bottomright:  #gives 0 for triangles so all are going to up1==up2 
                                    if up1==up2 or up2==bottomright:
                                        triangle= frontTriangle(x1, y1, z1, x2, y2, z1,up1[0],up1[1], up1[2])
                                        #trianglelist.append((triangle))
                                        
                                        """
                                        testarray0 = np.array([triangle.center[0]-ele[0] for ele in trianglecenters])
                                        testarray1 = np.array([triangle.center[1]-ele[1] for ele in trianglecenters])
                                        testarray2 = np.array([triangle.center[2]-ele[2] for ele in trianglecenters])
                                        closearray0 = np.isclose(testarray0, np.zeros(len(testarray0)))
                                        closearray1 = np.isclose(testarray1, np.zeros(len(testarray0)))
                                        closearray2 = np.isclose(testarray2, np.zeros(len(testarray0)))
                                        if closearray0.count(True)==0 and closearray1.count(True)==0 and closearray1.count(True)==0
                                        """ 
                                        testcounter=0
        
                                        for centercount in range(len(trianglecenters)):
                                            if np.abs(trianglecenters[centercount][0]-triangle.center[0])<tol and np.abs(trianglecenters[centercount][1]-triangle.center[1])<tol and np.abs(trianglecenters[centercount][2]-triangle.center[2])<tol:
                                                testcounter+=1
                                        if testcounter==0:
                                            trianglelist.append((triangle))
                                            trianglecenters.append(triangle.center)
                                            triangleadd+=1
                                            #print("try1")
                                            #print(i,j,l,ll)
                                            #print(x1, y1, z1, x2, y2, z1,up1[0],up1[1], up1[2])
                                        #else:
                                            #print("extra")
                                           # print(x1, y1, z1, x2, y2, z1,up1[0],up1[1], up1[2])
                                        st=1  
                                        ...
                                        testcounter2=0
                                        for centercount in range(len(trianglecenters)):
                                            if np.abs(trianglecenters[centercount][0]-triangle0.center[0])<tol and np.abs(trianglecenters[centercount][1]-triangle0.center[1])<tol and np.abs(trianglecenters[centercount][2]-triangle0.center[2])<tol:
                                                testcounter1+=1
                                            if np.abs(trianglecenters[centercount][0]-triangle1.center[0])<tol and np.abs(trianglecenters[centercount][1]-triangle1.center[1])<tol and np.abs(trianglecenters[centercount][2]-triangle1.center[2])<tol:
                                                testcounter2+=1
                                        
                                        if testcounter1== 0:
                                            trianglelist.append((triangle0))
                                            trianglecenters.append(triangle0.center)
                                        if testcounter2== 0:
                                            trianglelist.append((triangle1))
                                            trianglecenters.append(triangle1.center)
                                        #print(triangle0,triangle1)
                                        midind = up1[2]
                                        #print("try2")
                                        #print(x1,y1,z1,x2,y2,z1,up1[0],up1[1],up1[2])
                                        #print(x2,y2,z1,up2[0],up2[1],up2[2],up1[0],up1[1],up1[2])
...
class triangle:
    ...
    def center(x1,y1,z1,x2,y2,z2,x3,y3,z3):
        ...
        #if center1[0]==19 and center1[1]==5+1/3 and center1[2]==1/3:
        #    print("this is the triangle spot")
        return center1
    #@staticmethod
...
class Edgefinder:
    ...
    def fronts(self,pairslist,linesup):
        ...
        #linesup = ThreeDlineslist(pairslist,len(self.box[0]))
        #return pairslist
        #inbetweentriangles,flattopcount =findFronts(pairslist,linesup,len(self.box[0]))
        inbetweentriangles =findFronts(pairslist,linesup,len(self.box[0]))
        #trianglesfinal =CleanUpList(inbetweentriangles)
        #return trianglesfinal,flattopcount
        #self._fronts = trianglesfinal
        return inbetweentriangles
