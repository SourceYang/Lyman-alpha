...
int main(int argc, char **argv) {
  ...
  for(j=0; j<NGRID; j++) {
  	y1H[j] = 1. - 2e-4;
  	y1He[j] = 1. - 1e-10;
    EH[j] = T_i*1.5*(1.-y1H[j]+ABUND_HE*(1.- 1*y1He[j]))/RYD_K;
    EHI[j] = T_i*1.5*y1H[j]/RYD_K;
    EHII[j] = T_i*1.5*(1.-y1H[j])/RYD_K;
    EHeI[j] = T_i*1.5*(ABUND_HE*y1He[j])/RYD_K;
    EHeII[j] = T_i*1.5*(ABUND_HE*(1.-y1He[j]))/RYD_K;
  }
